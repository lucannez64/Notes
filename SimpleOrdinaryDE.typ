...
#label("simple-ordinary-differential-equation")
== First Order Linear Ordinary D.E
#label("first-order-linear-ordinary-d.e")
Let’s say that bunnies are procreating \
let x \= bunny population \
and the population grows at a rate $lambda$

$x^̇ eq frac(d x, d t) eq lambda x comma x lr((0)) med i n i t i a l med p o p med space i z e$
...
$ lambda eq frac(minus k / m plus.minus sqrt(lr((k / m))^2 minus 4 dot.op g / m), 2) dot $
...
need to use initial conditions, which specify the values of $x$ and $x^̇$
at a particular time $t_0$. Using these initial conditions, you can
solve for the constants $c_1$ and $c_2$ and obtain the specific solution
