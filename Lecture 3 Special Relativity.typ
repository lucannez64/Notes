...
#label("lecture-3-relativistic-laws-of-motion-and-emc2")
== Binomial Development
#label("binomial-development")
...
-  The relation between the proper time $tau$ and the coordinate time
  $t$ is given by: $ tau eq integral sqrt(1 minus v^2 / c^2) thin d t $
-  The proper time is always positive and is the longest possible time
  interval between two events.

== Worldline of a Particle
#label("worldline-of-a-particle")
...
-  The interval $d space^2$ along the worldline is given by:
  $ d space^2 eq c^2 thin d t^2 minus d x^2 minus d y^2 minus d z^2 $
...
#label("relation-of-4-velocity-to-regular-velocity")
...
== Least Action
#label("least-action")
-  The principle of least action states that the path taken by a
  particle between two events minimizes the action, which is the
  integral of the Lagrangian over the proper time:
  $ S eq integral L thin d tau $
...
#label("zero-mass-particles-and-positronium")
